...
namespace Server
{
    class server
    {
        ...
        static void Main(string[] args)
        {
            ...
            command = new sqliteCommand("create table if not exists table_hitLocation (X float, Y float, Player int)", conn);
            command.ExecuteNonQuery();
            command = new sqliteCommand("create table if not exists table_Crouch (X float, Y float, Player int)", conn);
            command.ExecuteNonQuery();
            command = new sqliteCommand("create table if not exists table_XRay (X float, Y float, Player int)", conn);
            command.ExecuteNonQuery();
            ...
            while (true)
            {
                ...
                if (messageToRead != "")
                {
                    ...
                    for (int i = 0; i < substrings.Length - 1; i++)
                    {
                        ...
                        // insert to table
                        var sql = "insert into " + table + " (x, y, player) values ";
                        sql += "('" + xValue + "'";
                        sql += ",";
                        sql += "'" + yValue + "'";
                        sql += ",";
